...
function GymApp({ onLogout }) {
  ...
  const addToWorkout = (exercise) => {
    ...
    if (exercises.find(ex => (ex.id || ex._id) === exerciseId)) { setShowCatalog(false); return; }
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      if (sessionData.exercises.length === 0) { setSaveResult("error"); setSaving(false); return; }
      await saveSession(sessionData);
      if (navigator.onLine) { syncAll().then(refreshPendingCount); } else { await refreshPendingCount(); }
      setSaveResult("ok");
      setTimeout(() => { setExercises([]); setActiveRoutineId(null); setSaveResult(null); }, 1500);
    } catch (err) {
      console.error(err);
      setSaveResult("error");
    } finally {
  ...
  const addExerciseToRoutine = (exercise) => {
    const exerciseId = exercise.id || exercise._id;
    if (routineExercises.find(ex => (ex.id || ex._id) === exerciseId)) { setShowRoutineCatalog(false); return; }
    const flatName = typeof exercise.name === 'object' ? exercise.name.es : exercise.name;
  ...
  return (
    <div className="flex-1 flex flex-col p-5 pb-50 max-w-281.5">
              ...
              <div className="space-y-3 mb-4">
                {ex.sets.map((set, index) => (
                  <div key={set.id} className="flex gap-2 sm:gap-3 items-center w-full">
                    <span className="text-zinc-600 font-mono text-[10px] w-4 flex-shrink-0 text-center">{index + 1}</span>
                    <input
